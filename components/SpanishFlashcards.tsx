...
import { buildSpanishExampleSentence } from "@/lib/spanish-words";
...
/**
 * Španělština — 10 random word/emoji flashcards shown to study, then the
 * same 10 quizzed back (emoji only, type the Spanish word), 1 XP per
 * correct guess. Mirrors EnglishFlashcards.tsx exactly, just against the
 * generateSpanishFlashcards/submitSpanishFlashcardAnswer callables.
 */
  ...
  if (phase === "study") {
    ...
    return (
      <div className="flex flex-col items-center gap-4 rounded-xl border border-border p-6">
        ...
        <p className="text-center text-sm italic text-zinc-500">{buildSpanishExampleSentence(card)}</p>
        <button
          type="button"
