...
import { collection, deleteField, doc, getDocs, query, updateDoc, where, writeBatch } from "firebase/firestore";
...
import { DEFAULT_PHOTO_COMPRESSION_QUALITY, DEFAULT_PHOTO_MAX_DIMENSION } from "@/lib/image-compress";
...
function closestPresetIndex(quality: number, maxDimension: number): number {
  ...
  QUALITY_PRESETS.forEach((p, i) => {
    const dist = Math.abs(p.quality - quality) + Math.abs(p.maxDimension - maxDimension) / 1000;
    if (dist < bestDist) {
      bestDist = dist;
      best = i;
    }
...
export default function PhotoSettingsPanel({
  ...
  async function handleClearPhotos() {
    const ok = await confirm({
      ...
      description: "Všechny nahrané fotky se nenávratně smažou. Úkoly samotné zůstanou beze změny.",
    ...
    try {
      const folderRef = storageRef(getFirebaseStorage(), `families/${familyId}/taskPhotos`);
      const { items } = await listAll(folderRef);
      await Promise.all(items.map((item) => deleteObject(item).catch(() => {})));

      const tasksWithPhotoSnap = await getDocs(
        query(collection(getDb(), "families", familyId, "dailyTasks"), where("photoUrl", ">", ""))
      );
      const docs = tasksWithPhotoSnap.docs;
      for (let i = 0; i < docs.length; i += 400) {
        const batch = writeBatch(getDb());
        for (const d of docs.slice(i, i + 400)) batch.update(d.ref, { photoUrl: deleteField() });
        await batch.commit();
      }

      if (user) logAction(familyId, user.uid, "photos_cleared", `Vymazáno ${items.length} fotek`);
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-1.5">
        <p className="text-sm text-zinc-500">
          Kvalita fotek při nahrávání — nižší kvalita šetří místo a je rychlejší na pomalém připojení.
        </p>
          ...
          {QUALITY_PRESETS.map((preset, i) => (
            <button
              ...
              className={`rounded-full px-3 py-1 disabled:opacity-50 ${
                currentIndex === i ? "bg-accent text-accent-foreground" : "text-zinc-500"
              }`}
            >
              {preset.label}
            </button>
      ...
      >
        <Trash2 size={16} /> {clearingPhotos ? "Mažu…" : "Vymazat všechny fotky"}
      </button>
    </div>
  );
}
