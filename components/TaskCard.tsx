...
const STATUS_EXPLANATIONS: Record<string, string> = {
  ...
  submitted:
    "Úkol byl odeslán ke schválení rodiči — XP se připíše až po schválení.",
  done: "Úkol je hotový a XP už bylo připsáno.",
  returned:
    "Rodič úkol vrátil zpět — podívej se na jeho poznámku a zkus to znovu.",
};
...
interface TaskCardProps {
  ...
  disabled?: boolean;
  /** false when a parent has turned off the whole proof-photo feature in Settings (family.photoRequirementsEnabled) — the template's own photoRequired stops counting either way. Defaults true so every other caller keeps its current behavior. */
...
function buildDetails(
  ...
  if (photosEnabled && template.photoRequired)
    parts.push("Vyžaduje přiložit foto jako důkaz.");
  if (STATUS_EXPLANATIONS[task.status])
    parts.push(STATUS_EXPLANATIONS[task.status]);
  if (task.status === "returned" && task.returnComment)
    parts.push(`Poznámka rodiče: ${task.returnComment}`);
  return parts.join(" ");
}
...
export default function TaskCard({
  ...
  const categoryIcon = template.category
    ...
    >
      ...
      <button
        ...
        onClick={() =>
          info({
            ...
            description: buildDetails(task, template, photosEnabled),
          })
        }
      ...
      >
        <div className="min-w-0 flex-1">
          <p
            className={`flex items-center gap-1 font-medium ${task.status === "done" ? "text-zinc-400 line-through" : ""}`}
          >
            ...
            {photosEnabled &&
              ...
              (task.status === "pending" || task.status === "returned") && (
                <Camera
                  ...
                  aria-label="Vyžaduje foto"
                />
              )}
          </p>
          {task.status === "returned" && task.returnComment ? (
            <p className="truncate text-sm text-danger">
              Vráceno: {task.returnComment}
            </p>
...
function StatusIcon({ status }: { status: DailyTask["status"] }) {
  if (status === "done")
    return <CheckCircle2 className="shrink-0 text-success" size={22} />;
  if (status === "submitted")
    return <Clock className="shrink-0 text-accent" size={22} />;
  if (status === "returned")
    return <RotateCcw className="shrink-0 text-danger" size={22} />;
