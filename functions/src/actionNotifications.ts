...
async function targetsFor(
  ...
  const snaps = await Promise.all(
    userIds.map((id) => familyRef.collection("members").doc(id).get()),
  );
  return snaps
    .filter((s) => s.exists)
    .map((s) => ({
      userId: s.id,
      fcmToken: (s.data() as Member | undefined)?.fcmToken,
    }));
}
...
export const onTaskProposalCreated = onDocumentCreated(
  ...
  async (event) => {
    ...
    await notifyMembers(
      ...
      targets,
      "Family Quest",
      `${proposerName} navrhl(a) nový úkol: „${proposal.title}“.`,
      db,
    );
  },
);
...
export const onTaskRequestOpened = onDocumentWritten(
  ...
  async (event) => {
    ...
    await notifyMembers(
      ...
      targets,
      "Family Quest",
      `${requesterName} chce nový úkol — navrhni mu/jí nějaký.`,
      db,
    );
  },
);
...
export const onXpAdjustmentRequestCreated = onDocumentCreated(
  ...
  async (event) => {
    ...
    const db = getFirestore();
    const familyRef = db.collection("families").doc(familyId);
    const parentsSnap = await familyRef
      .collection("members")
      .where("role", "==", "parent")
      .get();
    ...
    const otherParentIds = parentsSnap.docs
      .map((d) => d.id)
      .filter((id) => id !== request.requestedBy);
...
export const onPooledContributionCreated = onDocumentCreated(
  ...
  async (event) => {
    ...
    await notifyMembers(
      ...
      targets,
      "Family Quest",
      `Zveme tě do sbírky na „${reward?.title ?? "odměnu"}“ — přispěj svým dílem XP.`,
      db,
    );
  },
);
...
export const onRewardRedemptionActionable = onDocumentWritten(
  ...
  async (event) => {
    ...
    const rewardSnap = await familyRef
      .collection("rewards")
      .doc(after.rewardId)
      .get();
    const reward = rewardSnap.data() as Reward | undefined;
...
export const onMarketplaceOfferActionable = onDocumentWritten(
  ...
  async (event) => {
    ...
    if (!before) {
      ...
      const responderId =
        before.lastActionBy === before.proposedBy
          ? before.targetUserId
          : before.proposedBy;
...
export const onRewardRequestActionable = onDocumentWritten(
  "families/{familyId}/rewardRequests/{requestId}",
  async (event) => {
    const before = event.data?.before.data() as RewardRequest | undefined;
    const after = event.data?.after.data() as RewardRequest | undefined;
    if (!after) return;
    ...
    if (!before && after.status === "pending") {
      ...
      const otherParentIds = parentsSnap.docs
        .map((d) => d.id)
        .filter((id) => id !== after.requestedBy);
      const targets = await targetsFor(db, familyId, otherParentIds);
      if (targets.length === 0) return;

      const requesterName = await memberName(db, familyId, after.requestedBy);
      await notifyMembers(
        familyId,
        "reward_request",
        targets,
        "Family Quest",
        `${requesterName} navrhuje novou odměnu: „${after.title}“.`,
        db,
      );
      return;
    }
    ...
    if (
      ...
      const targets = await targetsFor(db, familyId, [after.requestedBy]);
      if (targets.length === 0) return;

      const message =
        after.status === "approved"
          ? `Tvůj návrh odměny „${after.title}“ byl schválen! Stojí ${formatXp(after.xpCost ?? 0)} XP.`
          : `Tvůj návrh odměny „${after.title}“ byl zamítnut.`;
      await notifyMembers(
        familyId,
